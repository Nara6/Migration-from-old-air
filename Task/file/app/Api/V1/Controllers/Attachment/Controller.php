...
class Controller extends ApiController
{
    ...
    public function index(Request $request){
        ...
        */
        $project_folder_path = 'uploads/'.$project;
    ...
    public function imageResize(Request $request){
        ...
        */
        ...
        // or create a new image resource from binary data
        // file_put_contents($uri, $image_base64);
        $img = Image::make('uploads/'.$project.'/'.$folder.'/'.$fileName.'.'.$extension);
        $img->resize('300','500', function($constraint) {
            $constraint->aspectRatio();
        });
        $img->save('uploads/'.$project.'/'.$folder.'/'.$fileName.'11'.'.'.$extension);
        $uriResize = 'uploads/'.$project.'/'.$folder.'/'.$fileName.'11'.'.'.$extension;
        /**
         * Check if resize existing
         */
        $resizes_data = '';
