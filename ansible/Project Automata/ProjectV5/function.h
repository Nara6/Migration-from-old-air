...
void testString(List* ArrayOfState[], Queue *Q , string str, int n, int final[], int n_final ,int index){
    ...
    if(index == str.length()){
        if(isFinal(Q, n_final, final)){   // check if the Queue contains final state
            cout << "\nString accepted";  // Queue contains final state then string accepted
            //displayQueue(Q);
        }else{
            cout << "\nString not accepted";
            //displayQueue(Q);
...
void SaveData(int fa_id, List **ArrayOfState, int n, int n_symbol, char *symbol, int *final, int n_final, int m, int current_state_DB[], char symbol_DB[], int next_state_DB[]){
    ...
    char ChangeDes;
    cout<<"Do you want to change description of this FA? (Y/N): "; cin>>ChangeDes; fflush(stdin);
    ...
    if(toupper(ChangeDes) == 'Y'){
        ...
        cout<<"Description is not changed."<<endl;
    }
...
int arrowKeyChoice(char choice[10][50], char header[50], int n){
    ...
    while(1){
        ...
        else if (y1 == 0xE0) {    // a scroll key was pressed
            y2 = getch();          // determine what it was
            system("cls");
