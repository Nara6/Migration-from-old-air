...
public class PurchaseTest {
    public static void main(String[] args) {
        PurchaseORM purchase = new PurchaseORM();
        int option;
        do{
            ...
            switch(option) {
                ...
                System.out.println("|ID\t\tTotal_Price\t\tSupplier_id\t\tUser_id\n");
                for(var c:purchase.listAll()){
                    System.out.println(c.getId()+"\t\t"+c.getTotal_Price()+"\t\t\t"+c.getSupllier_id()+"\t\t"+c.getUser_id());
                }
                break;
                case 2:
                System.out.println("2. CREATE NEW RECORD");
                ...
                var sa = new Purchase(0, total_price,supplier_id,user_id);
                purchase.add(sa);
                System.out.println("|ID\t\tTotal_Price\t\tSupplier_id\t\tUser_id\n");
                for(var c:purchase.listAll()){
                    System.out.println(c.getId()+"\t\t"+c.getTotal_Price()+"\t\t\t"+c.getSupllier_id()+"\t\t"+c.getUser_id());
                ...
                if(hold==0){
                    ...
                    int new_supplier_id = sc.nextInt();
                    System.out.println("\t!CAUTION PLEASE INPUT THE EXISTING USER ID.");
                    System.out.print("\n\t*USER ID: ");
                    while(!sc.hasNextInt()){
                        System.out.print("\n\t***PLEASE INPUT ONLY NUMBER: ");
                    ...
                    var newPurchase = new Purchase(id,new_total_price,new_supplier_id,new_user_id);
                    purchase.update(newPurchase);
                    System.out.println("|ID\t\tTotal_Price\t\tSupplier_id\t\tUser_id\n");
                    for(var c:purchase.listAll()){
                        System.out.println(c.getId()+"\t\t"+c.getTotal_Price()+"\t\t\t"+c.getSupllier_id()+"\t\t"+c.getUser_id());
                    }
                ...
                int Did = sc.nextInt();
                boolean isDelete = purchase.delete(Did);
                if(isDelete){
