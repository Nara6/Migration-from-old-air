    ...
    function uploaddoc(Request $request){
        ...
        if($request->ext == 'pdf'){
            ...
            if($request->file){
                ...
                $folder = isset($request->folder)?$request->folder:"public/uploads/unknown/";
                ...
                // $image_type_aux = explode("pdf/", $image_parts[0]);
            
                // $extension = $image_type_aux[0];
                
                // $image_base64 = base64_decode($image_parts[0]);
                // return $image_base64;
                $fileName = isset($request->fileName)?$request->fileName: uniqid();
                $image_parts = explode(";base64,", $request->file);
                $image_base64 = base64_decode($image_parts[1]);
                
                $uri = $folder.$fileName.'.'.$request->ext; 
                file_put_contents($uri, $image_base64); 
                ...
                if( isset($request->sizes) ){
                    $this->fit($folder, $fileName, $request->ext); 
                }
