 ...
 */
  ...
  lstColumnTableNames = ['រូបភាព', 'ឈ្មោះផលិតផល', 'តម្លៃ', 'សកម្មភាព'];
  columnTableInfo: any = [];
  ...
  createColumnTblName() {
    ...
    if (this.lstColumnTableNames && this.lstColumnTableNames.length) {
      ...
      let lstTamplates = [this.skuTpl, this.colName, this.colPrice, this.colAction, this.colAction];
  ...
  listing(params: any = {}) {
    ...
    if (this.isSearching) {
      ...
      this._service.rank(params).subscribe(res => {
