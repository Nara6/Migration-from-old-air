...
namespace App\Api\V1\Controllers\MyProfile;
...
class MyProfileController extends ApiController
{
    ...
    function get(){
        ...
        $admin = Main::select('id','uid','name','phone','email','avatar')->where('id', $auth->id)->first();
    ...
    function put(Request $request){
        ...
        $res        = FileUpload::uploadFile($request->avatar, 'my-profile/' . $today, '');
        if ($res) {
            if (isset($res['url'])) {
                if($res['url'] != ''){
                    $user->avatar          = $res['url'];
