  ...
  <div class="content">
    <v-toolbar flat>
      ...
      <v-dialog v-model="dialog" max-width="500px">
        ...
        <v-card>
          ...
          <v-card-actions>
            ...
            <v-btn color="blue darken-1" text @click="save" :disabled="!form"> Save </v-btn>
          </v-card-actions>
        </v-card>
      </v-dialog>
      ...
      <v-dialog v-model="viewDialog" max-width="550px">
        <v-card v-for="item in oneItem" :key="item._id">
          <v-card-text>
            <v-container>
              <v-row>
                <v-col>
                  ...
                  <v-row>
                    ...
                    <v-col class="view-detail-right">
                      {{ deadlineDate(item.pickUpDate, item.duration) }}
                    </v-col>
                  </v-row>
                  <v-row>
                    <v-col class="view-detail-left">
                    ...
                    <v-col class="view-detail-right" :class="statusColor(item.status)">
                      {{ item.status }}
                    </v-col>
                  </v-row>
                  <v-row v-show="isApprove">
                    <v-col class="view-detail-left">
                      <h4>Actions :</h4>
                    </v-col>
                    <v-col class="view-detail-right" :class="statusColor(item.status)">
                      <v-icon small class="mr-6" @click="approve(item._id, false)">
                        mdi-window-close
                      </v-icon>
                      <v-icon small class="mr-2" @click="approve(item._id, true)">
                        mdi-check
                      </v-icon>
                    </v-col>
                  </v-row>
...
export default {
  ...
  computed: {
    ...
    computedDateFormattedDatefns() {
      return this.editedItem.pickUpDate ? format(parseISO(this.editedItem.pickUpDate), "EEEE, MMMM do yyyy") : undefined;
    },
  },
  ...
  methods: {
    ...
    async approve(id ,accept){
      ...
      const result = await borrowApi.update(id, formData)
      if(result.status === 200){
        if(accept === true){
          this.errorMessage = "Approved Successfully"
          this.isSnackbar = true
        }else{
          this.errorMessage = "Rejected Successfully"
          this.isSnackbar = true
        }
      }else{
        this.errorMessage = result.data
